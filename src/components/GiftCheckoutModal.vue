                ...
                <div class="grid w-full grid-cols-1 items-start gap-x-6 gap-y-8 sm:grid-cols-12 lg:gap-x-8">
                  <div class="sm:col-span-4 lg:col-span-5">
                    <div class="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg bg-gray-100">
                      <img :src="gift!.image!" :alt="gift!.title!" class="object-cover object-center" />
                    </div>
                  </div>
                  <div class="sm:col-span-8 lg:col-span-7">
                    ...
                    <section aria-labelledby="options-heading" class="mt-6">
                      ...
                      <form>
                        <GiftPrice :total-price="gift!.price!" :gifted-amount="gift!.amount_gifted" :giftable="false"
                          v-if="pricingMode" />
                        <GiftParts :total-parts="gift!.parts!" :gifted-parts="gift!.amount_gifted" :giftable="false"
                          :part-price="gift!.price!" v-else />
                        ...
                        <div class="flex justify-between mt-2">
                          ...
                          <div class="relative rounded-md shadow-sm" v-if="pricingMode">
                            <input type="text" name="price" id="price"
                              ...
                              placeholder="0.00" aria-describedby="price-currency" v-model="amount" min="0"
                              :max="remainingAmount" />
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                              <span class="text-gray-500 sm:text-sm" id="price-currency">{{ settingsStore.currency
                                }}</span>
                            </div>
                          </div>
                          ...
                          <div class="relative rounded-md shadow-sm" v-else>
                            <select id="parts" name="parts" v-model="amount"
                              ...
                              <option v-for="i in remainingAmount" :key="i" :value="i">{{ i }}</option>
                            </select>
                          </div>
                        ...
                        <div class="mt-6 flex space-x-2 w-full">
                          <button type="submit" @click.prevent="addToCart(false)"
                            class="items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50">{{
                              t('gift.add_to_cart_checkout') }}</button>
                          <button type="submit" @click.prevent="addToCart(true)"
                            class="w-full items-center justify-center rounded-md border border-transparent bg-gray-200 px-8 py-3 text-base font-medium text-gray-900 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50">{{
                              t('gift.add_to_cart_continue') }}</button>
                        </div>
                      </form>
                    </section>
                  </div>
...
const remainingAmount = computed(() => {
  if (pricingMode.value)
    return gift.value!.price! - gift.value!.amount_gifted!
...
function addToCart(continueShopping: boolean = false) {
  ...
  if (pricingMode.value) {
    cartStore.addToCart({ id: gift.value!.id!, title: gift.value!.title!, quantity: 1, cost: amount.value, img: gift.value!.image!, mode: 'price' })
  } else {
    cartStore.addToCart({ id: gift.value!.id!, title: gift.value!.title!, quantity: amount.value, cost: gift.value!.price!, img: gift.value!.image!, mode: 'parts' })
  }
